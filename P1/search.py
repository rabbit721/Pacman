...
def traverse(goal):
    ...
    curr_item = goal
    while(curr_item):
        result.insert(0, curr_item.action)
        last_item = curr_item.parent
        curr_item = last_item
    result.pop(0)
    return result
...
def dfs(problem, max):
    ...
    start = Node(st, None, None, 1)
    frontier.append(start)
    while frontier:
        curr_item = frontier.pop()
        curr = curr_item.state
        if(problem.goalTest(curr)):
            return (True, traverse(curr_item))
        if(curr_item.path_cost == max):
            continue
        explored.append(curr_item)
        for act in problem.getActions(curr):
            next = problem.getResult(curr, act)
            node = Node(next, curr_item, act, curr_item.path_cost + 1)
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    st = problem.getStartState()
    start = Node(st, None, None, 0)
    pq.push(start,heuristic(st, problem))
    frontier.append(st)
    while pq:
        curr_item = pq.pop()
        curr = curr_item.state
        p_curr = curr_item.path_cost
        frontier.remove(curr)
        if(problem.goalTest(curr)):
            return traverse(curr_item)
        explored.append(curr)
        #print("curr:", curr, "  h:" , heuristic(curr, problem))
        for act in problem.getActions(curr):
            next = problem.getResult(curr, act)
            c = p_curr + problem.getCost(curr, act)
            p = c + heuristic(next, problem)
            node = Node(next, curr_item, act, c)
            #print("next:", next, "  p:", p)
