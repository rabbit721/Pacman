...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        "*** YOUR CODE HERE ***"
        #position(x,y);
        print(self.startingPosition, self.food)
...
def cornersHeuristic(state, problem):
    ...
    if (d == 0):
        ...
        h = m + min(walls.height, walls.width) + walls.height + walls.width - 9
    elif(foods[1] == foods[2] or foods[0] == foods[3]):
        h = m + walls.height + walls.width - 6
    elif((foods[0] and foods[1]) or (foods[2] and foods[3])):
        h = m + walls.height - 3
    elif((foods[0] and foods[2]) or (foods[1] and foods[3])):
        h = m + walls.width - 3

    "*** YOUR CODE HERE ***"
    print("curr:", state[0], "foods: ", foods, "  h:" , h)
...
def foodHeuristic(state, problem):
        ...
        for x in range(0, width + 1):
            if(foodGrid[x][y]):
                ...
                if (anchor > -1):
                    sum += x - anchor
                anchor = x
                m = min(m, manhattan(position,(x,y)))
    ...
    if(d > 0):
        d -= 1
    cords = foodGrid.asList()
    if (len(cords) > 1):
        h2 = manhattan(cords[0], cords[-1])
        return max(h2, d + m)
    #print("curr:", position, ";m: ", m,";  h:" , d + m)
    return d + m
