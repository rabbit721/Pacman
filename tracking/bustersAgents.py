...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for d in livingGhostPositionDistributions:
            ...
            (pos, val) = lst[-1]
            dist = self.distancer.getDistance(pos, pacmanPosition)
            if (dist < min):
                min = dist
                best = pos
        ...
        for act in legal:
            ...
            if (self.distancer.getDistance(successorPosition, best) < min):
                min = self.distancer.getDistance(successorPosition, best)
                bestAct = act
        #print("dope")
        if bestAct is None:
            return Directions.STOP
